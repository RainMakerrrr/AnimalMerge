    ...
    internal class GameAnalyticsProvider : IAnalyticsProvider
    {
        ...
        private static void OnGameFinished(GameFinishedParameters parameters)
        {
            GameAnalyticsWrapper.TrackProgressEvent(
                parameters.status ? GAProgressionStatus.Complete : GAProgressionStatus.Fail, parameters.level,
        ...
        {
            GameAnalytics.NewAdEvent(GAAdAction.Show, GAAdType.Interstitial, adAnalyticsInfo.AdNetworkName,
                adAnalyticsInfo.adPlacement);
        }

        private static void OnInterstitialClickedEvent(AdClickEventAnalyticsInfo adAnalyticsInfo)
        {
            GameAnalytics.NewAdEvent(GAAdAction.Clicked, GAAdType.Interstitial, adAnalyticsInfo.AdNetworkName,
                adAnalyticsInfo.adPlacement);
        }
        ...
        {
            GameAnalytics.NewAdEvent(GAAdAction.Show, GAAdType.RewardedVideo, adAnalyticsInfo.AdNetworkName,
                adAnalyticsInfo.adPlacement);
        }

        private static void OnRewardedClickedEvent(AdClickEventAnalyticsInfo adAnalyticsInfo)
        {
            GameAnalytics.NewAdEvent(GAAdAction.Clicked, GAAdType.RewardedVideo, adAnalyticsInfo.AdNetworkName,
                adAnalyticsInfo.adPlacement);
        }
    }
}