    ...
    void find_next_path_static(character tchar, tile ttile, List<tile> db_tpath)
    {
        for (int x = 0; x < ttile.db_neighbors.Count; x++)
        {
            ...
            if (ttile.db_neighbors[x].tile_s != null && !db_tpath.Contains(ntile) &&
                !ttile.db_neighbors[x].blocked) //Check if tile, if not already used, if not blocked//
            {
                if (find_path == efind_path.once_per_turn || (db_tpath.Count < tchar.max_tiles))
                {
                    if (ntile.db_path_lowest.Count == 0 || db_tpath.Count + 1 < ntile.db_path_lowest.Count)
                    {
                        if ((!tchar.big) || (tchar.big && ntile.db_neighbors[1].tile_s != null &&
                                             !ntile.db_neighbors[1].blocked && ntile.db_neighbors[2].tile_s != null &&
    ...
    void find_next_path_realtime(character tchar, tile ttile, List<tile> db_tpath, tile tar_tile_s)
    {
        for (int x = 0; x < ttile.db_neighbors.Count; x++)
        {
            ...
            if (ttile.db_neighbors[donum].tile_s != null && !db_tpath.Contains(ntile) &&
                !ttile.db_neighbors[donum].blocked) //Check if tile, if not already used, if not blocked//
            {
